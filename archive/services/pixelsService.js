...
async function getAll(orderBy, fields = ['*']) {
  const pixels = await db
    .select('fb_pixels.' + fields, {
      token: 'fb_pixels.token',
      name: 'fb_pixels.name',
...
async function add(pixelData, fields = ['*']) {
  const newPixel = await db('fb_pixels')
    .insert(pixelData)
    .returning(...fields);
  return newPixel;
}
...
async function update(pixel_id, updateData, fields = ['*']) {
  const updated = await db('fb_pixels')
    .where({ pixel_id })
    .first()
    .update(updateData)
    .returning(...fields);
