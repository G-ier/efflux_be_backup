...
class InsightRepository {
  ...
  constructor() {
    ...
    parseMediaNetAPIData(insight) {
        ...
        let [pixel_id, campaign_id, adset_id, ad_id, traffic_source, external] = insight.channelName2 ? insight.channelName2.split('_|_') : 
        ['Unknown', 'Unknown', 'Unknown', 'Unknown', 'Unknown', 'Unknown'];
        let [ip, country_code, region, city, timestamp, campaign_name, session_id] = insight.channelName3 ? (insight.channelName3).split('_|_') : 
        ['Unknown', 'Unknown', 'Unknown', 'Unknown', 'Unknown', 'Unknown', 'Unknown'];
        
        session_id = session_id && /^[0-9a-zA-Z]+$/.test(session_id) ? session_id : 'Unknown';
        const [date, hour] = timestamp ? extractDateHourFromUnixTimestamp(timestamp): ['1970-01-01', 0];
