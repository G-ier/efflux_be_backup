...
class CapiService extends BaseService {
  ...
  async createCapiLogEntry(data) {
    ...
    const logEntries = data.map((event) => {
      const pst_timestamp = moment
        .utc(event.timestamp * 1000)
        .tz('America/Los_Angeles')
        .format('YYYY-MM-DDTHH:mm:ss');
      const constructed_fbc = !['', 'undefined', null, undefined].includes(event.fbc)
        ? false
        : true;
      const constructed_fbp = !['', 'undefined', null, undefined].includes(event.fbp)
        ? false
        : true;
  ...
  constructFacebookConversionEvents(events) {
    ...
    events.forEach((event) => {
      if (
        ['', null, undefined].includes(event.timestamp) ||
        ['', null, undefined].includes(event.external)
      ) {
        return;
      }

      const fbc = !['', 'undefined', null, undefined].includes(event.fbc)
        ? event.fbc
        : `fb.1.${event.timestamp * 1000}.${event.external}`;
      const fbp = !['', 'undefined', null, undefined].includes(event.fbp)
        ? event.fbp
        : `fb.1.${event.timestamp * 1000}.${generateEventId()}`;
      const state =
        event.country_code === 'US' && usStates[event.state.toUpperCase()] !== undefined
  ...
  async updateInvalidEvents(brokenEvents, network = 'crossroads') {
    ...
    const updatedCount = await this.database.update(
      tableName,
      { valid_pixel: false },
