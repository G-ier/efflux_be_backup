...
const disableFacebookCron =
  process.env.DISABLE_FACEBOOK_CRON === "true" || process.env.DISABLE_FACEBOOK_CRON !== "false";
...
const updateYesterdayDataBeforeMidnightPST = new CronJob(FACEBOOK_UPDATE_YESTERDAY_BEFORE_MIDNIGHT_CRON, async () => {
  await updateFacebookData("yesterday");
});

const updateYesterdayDataAfterMidnightPST = new CronJob(FACEBOOK_UPDATE_YESTERDAY_AFTER_MIDNIGHT_CRON, async () => {
  await updateFacebookData("yesterday");
});

const updateYesterdayDataAfterMidnightPST2 = new CronJob(FACEBOOK_UPDATE_YESTERDAY_AFTER_MIDNIGHT_2_CRON, async () => {
  await updateFacebookData("yesterday");
});
