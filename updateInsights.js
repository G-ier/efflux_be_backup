...
async function updateInsightsOnDatabase(data, trafficSource) {
  ...
  for (const chunk of dataChunks) {
    ...
    // await db('insights').insert(chunk)
...
async function updateInsightsJob(day = "today") {
...
const InsightsRegularJob = new CronJob(
  '0 0 * * *', // This will be changed
  updateInsightsJob.bind(null, "today"),
);

const InsightsYesterdayJob = new CronJob(
  '0 0 * * *', // This will be changed
  updateInsightsJob.bind(null, "yesterday"),
);
...
const initializeInsightsCron = async () => {
  ...
  if (!disableCron) {
    InsightsRegularJob.start();
    InsightsYesterdayJob.start();
  }
